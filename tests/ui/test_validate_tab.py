...
class TestValidateTabTokenSummary:
    """Tests for Issue #38 - token transparency in validate_tab."""

    def test_source_imports_render_token_summary(self):
        """Validate tab imports render_token_summary from components."""
        source = inspect.getsource(validate_tab_module)
    ...
    def test_source_imports_estimate_tokens(self):
        """Validate tab imports estimate_tokens from components."""
        source = inspect.getsource(validate_tab_module)
        assert "estimate_tokens" in source

    def test_source_has_estimated_request_tokens_state_key(self):
        """Validate tab references VALIDATE_ESTIMATED_REQUEST_TOKENS state key."""
        source = inspect.getsource(validate_tab_module)
    ...
    def test_prompt_only_mode_calls_render_token_summary(self):
        ...
        source = inspect.getsource(validate_tab_module)
        prompt_only_idx = source.find("def _render_validate_prompt_only")
    ...
    def test_prompt_only_mode_stores_estimated_request_tokens(self):
        ...
        state_set_idx = source.find(
            "st.session_state[VALIDATE_ESTIMATED_REQUEST_TOKENS]", prompt_only_idx
        )
    ...
    def test_api_mode_calls_render_token_summary_before_run_button(self):
        ...
        source = inspect.getsource(validate_tab_module)
        api_idx = source.find("def _render_validate_api")
        run_btn_idx = source.find('st.button("Run validation"', api_idx)
        render_idx = source.find("render_token_summary", api_idx)
        assert render_idx != -1, "render_token_summary not found in API mode"
        assert run_btn_idx != -1, "Run validation button not found"
